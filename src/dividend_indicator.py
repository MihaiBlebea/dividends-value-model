...
class DividendIndicator:
	...
	def get_dividend_yield(self)-> float:
		...
		div_yield = safeget(data, "quoteSummary", "result", 0, "summaryDetail", "dividendYield", "raw")
	...
	def get_yearly_ratios(self)-> list:
		...
		statements = safeget(data, "quoteSummary", "result", 0, "cashflowStatementHistory", "cashflowStatements")
		...
		return [
			{
				...
				"payout_ratio": abs(safeget(sts, "dividendsPaid", "raw")) / safeget(sts, "netIncome", "raw"),
				"dividend_cover": safeget(sts, "netIncome", "raw") / abs(safeget(sts, "dividendsPaid", "raw"))
			} 
